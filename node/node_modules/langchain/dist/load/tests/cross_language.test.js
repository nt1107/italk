...
const IMPORTANT_IMPORTS = JSON.parse(readFileSync(path.join(path.dirname(fileURLToPath(import.meta.url)), "data", "important_imports.json")).toString());
...
describe("Test cross language serialization of important modules", () => {
    ...
    test.each(CROSS_LANGUAGE_ENTRYPOINTS)("Test matching serialization names for: %s", async (item) => {
        ...
        try {
            ...
            const result = (await load(JSON.stringify(mockItem)));
