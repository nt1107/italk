...
const runTest = async ({ modelName, config, system = "", message = "Hello!", shouldThrow = false, }) => {
    const description = `Test ChatAlibabaTongyi ${modelName || "default model"} ${config.description || ""}`.trim();
    let nrNewTokens = 0;
    ...
    test.skip(description, async () => {
        ...
        console.log({ res });
        // test streaming call
        const stream = await chat.stream(`Translate "I love programming" into Chinese.`);
        const chunks = [];
