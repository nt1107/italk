...
describe("PGVectorStore", () => {
    ...
    test("PGvector can save documents with a list greater than default chunk size", async () => {
        try {
            ...
            const result = await pgvectorVectorStore.pool.query(`SELECT COUNT(*) FROM "${tableName}"`);
    ...
    test("PGvector can save documents with ids", async () => {
        ...
        const result = await pgvectorVectorStore.pool.query(`SELECT id FROM "${tableName}" WHERE id = $1`, [id1]);
    ...
    test("PGvector can delete document by id", async () => {
        try {
            ...
            await pgvectorVectorStore.addDocuments(documents);
            const result = await pgvectorVectorStore.pool.query(`SELECT id FROM "${tableName}"`);
            ...
            const result2 = await pgvectorVectorStore.pool.query(`SELECT id FROM "${tableName}"`);
...
describe("PGVectorStore with collection", () => {
    ...
    afterEach(async () => {
        // Drop table, then recreate it for the next test.
        await pgvectorVectorStore.pool.query(`DROP TABLE "${tableName}"`);
        await pgvectorVectorStore.pool.query(`DROP TABLE ${pgvectorVectorStore.computedCollectionTableName}`);
        await pgvectorVectorStore.ensureTableInDatabase();
    ...
    test("'name' column is indexed", async () => {
        const result = await pgvectorVectorStore.pool.query(`SELECT * FROM pg_indexes WHERE tablename = '${pgvectorVectorStore.computedCollectionTableName}'`);
...
describe("PGVectorStore with schema", () => {
    ...
    let computedCollectionTableName;
    let pool;
    beforeAll(async () => {
    ...
    afterEach(async () => {
        ...
        await pgvectorVectorStore.pool.query(`DROP TABLE ${computedCollectionTableName}`);
        await pgvectorVectorStore.ensureTableInDatabase();
        await pgvectorVectorStore.ensureCollectionTableInDatabase();
    });
    afterAll(async () => {
    ...
    test("Test table creation with schema", async () => {
        const result = await pgvectorVectorStore.pool.query(`SELECT table_schema FROM information_schema.tables WHERE table_name = '${tableName}' AND table_schema = '${schema}'`);
        expect(result.rowCount).toEqual(1);
        const result2 = await pgvectorVectorStore.pool.query(`SELECT table_schema FROM information_schema.tables WHERE table_name = '${collectionTableName}' AND table_schema = '${schema}'`);
        expect(result2.rowCount).toEqual(1);
    });
    test("Test embeddings creation", async () => {
        const documents = [
    ...
    test("PGvector can save documents with a list greater than default chunk size", async () => {
        try {
            ...
            await pgvectorVectorStore.addDocuments(documents);
            // Query the table to check the number of rows
            const result = await pgvectorVectorStore.pool.query(`SELECT COUNT(*) FROM ${computedTableName}`);
            const rowCount = parseInt(result.rows[0].count, 10);
            // Check if the number of rows is equal to the number of documents added
            expect(rowCount).toEqual(docsToGenerate);
        }
    ...
    test("PGvector can save documents with ids", async () => {
        ...
        await pgvectorVectorStore.addDocuments(documents, { ids: [id1, id2] });
        const result = await pgvectorVectorStore.pool.query(`SELECT id FROM ${computedTableName} WHERE id = $1`, [id1]);
        expect(result.rowCount).toEqual(1);
    });
    test("PGvector supports different filter types", async () => {
        const documents = [
    ...
    test("PGvector can delete document by id", async () => {
        try {
            ...
            await pgvectorVectorStore.addDocuments(documents);
            const result = await pgvectorVectorStore.pool.query(`SELECT id FROM ${computedTableName}`);
            const initialIds = result.rows.map((row) => row.id);
            const firstIdToDelete = initialIds[0];
            const secondIdToDelete = initialIds[1];
            const idToKeep = initialIds[2];
            ...
            const result2 = await pgvectorVectorStore.pool.query(`SELECT id FROM ${computedTableName}`);
            // Only one row should be left
            expect(result2.rowCount).toEqual(1);
            // The deleted ids should not be in the result
            const idsAfterDelete = result2.rows.map((row) => row.id);
...
describe("PGVectorStore with HNSW index", () => {
    ...
    test("Ensure table has HNSW index", async () => {
        const result = await pgvectorVectorStore.pool.query(`SELECT indexname, tablename, indexdef FROM pg_indexes where indexname='vector_embedding_hnsw_idx';`);
