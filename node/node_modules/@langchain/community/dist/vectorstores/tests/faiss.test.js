...
test("Test FaissStore.fromIndex + mergeFrom", async () => {
    ...
    const results1 = await vectorStore2.similaritySearchVectorWithScore([1, 0, 0, 0], 2);
    expect(results1).toHaveLength(2);
    expect(results1).toEqual([
        [new Document({ metadata: { id: 1 }, pageContent: "my world" }), 0],
        [new Document({ metadata: { id: 2 }, pageContent: "our world" }), 1],
    ]);
    const vectorStore3 = await FaissStore.fromIndex(vectorStore2, new FakeEmbeddings());
    const results2 = await vectorStore3.similaritySearchVectorWithScore([1, 0, 0, 0], 2);
...
test("Test FaissStore Exceptions", async () => {
    const vectorStore = new FaissStore(new FakeEmbeddings(), {});
    expect(() => vectorStore.index).toThrow("Vector store not initialised yet. Try calling `fromTexts`, `fromDocuments` or `fromIndex` first.");
    ...
    await vectorStore2.addVectors([[1, 1, 1]], [
        new Document({
    ...
    await expect(async () => {
        ...
        await vectorStore3.delete({ ids: null });
