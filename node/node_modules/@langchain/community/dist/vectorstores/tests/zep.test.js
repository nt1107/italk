...
import { ZepVectorStore } from "../zep.js";
...
const mockZepDocuments = mockDocuments.map((doc, index) => ({
    uuid: `uuid${index}`,
...
const mockCollection = {
    ...
    search: jest
        .fn()
        .mockResolvedValue(mockZepDocuments),
    deleteDocument: jest
        .fn()
        .mockResolvedValue(undefined),
    searchReturnQueryVector: jest
        .fn()
        .mockResolvedValue([mockZepDocuments, new Float32Array([0.0, 0.1])]),
    name: "testCollection",
...
const mockClient = {
    document: {
        ...
        addCollection: jest.fn().mockResolvedValue(mockCollection),
    },
};
function isADocument(obj) {
...
describe("ZepVectorStore", () => {
    ...
    test("should add documents successfully", async () => {
        ...
        zepVectorStore.collection = mockCollection;
    ...
    test("should create ZepVectorStore from texts successfully", async () => {
        ...
        ZepVectorStore.fromDocuments = jest.fn(async (docs, embeddings, zepConfig) => {
            const zepVectorStore = await originalFromDocuments.call(ZepVectorStore, docs, embeddings, zepConfig);
            zepVectorStore.collection = mockCollection;
            return zepVectorStore;
        });
        const zepVectorStore = await ZepVectorStore.fromTexts(texts, metadatas, embeddings, zepConfig);
        ...
        mockCollection.addDocuments.mock.calls[0][0].forEach((obj) => {
    ...
    test("should perform similarity search with score successfully", async () => {
        ...
        const result = await zepVectorStore.similaritySearchVectorWithScore(query, k, filter);
        expect(mockCollection.search).toHaveBeenCalledWith(expect.objectContaining({
    ...
    test("should perform similarity search successfully", async () => {
        ...
        const docs = mockZepDocuments.map((doc) => new Document({
            pageContent: doc.content,
            metadata: doc.metadata,
        }));
