...
test("SupabaseVectorStore with external ids", async () => {
    const client = createClient(process.env.SUPABASE_VECTOR_STORE_URL, process.env.SUPABASE_VECTOR_STORE_PRIVATE_KEY);
...
test("Upsert on a SupabaseVectorStore", async () => {
    ...
    expect(results).toHaveLength(1);
    const ids2 = await store.addDocuments([{ pageContent: "hello 1", metadata: { created_at: createdAt } }], { ids });
