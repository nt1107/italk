...
const getDocs = () => {
    ...
    const documents = [
        new Document({
            ...
            metadata: {
                document_id: insecureHash(englishOne),
        ...
        new Document({
            ...
            metadata: {
                document_id: insecureHash(frenchOne),
...
describe("VectaraStore", () => {
    ["VECTARA_CUSTOMER_ID", "VECTARA_CORPUS_ID", "VECTARA_API_KEY"].forEach((envVar) => {
    ...
    describe("fromTexts", () => {
        ...
        test.skip("with fakeEmbeddings doesn't throw error", () => {
            expect(() => VectaraStore.fromTexts([], [], new FakeEmbeddings(), args)).not.toThrow();
        });
    });
    describe("fromDocuments", () => {
        ...
        test.skip("with fakeEmbeddings doesn't throw error", async () => {
            await expect(VectaraStore.fromDocuments(getDocs(), new FakeEmbeddings(), args)).resolves.toBeDefined();
        });
    });
    describe("access operations", () => {
        ...
        test.skip("similaritySearchWithScore", async () => {
            const resultsWithScore = await store.similaritySearchWithScore("What did Sam do?", 10, // Number of results needed
            { lambda: 0.025 });
        ...
        test.skip("similaritySearch", async () => {
            const results = await store.similaritySearch("Was Gandalf dead?", 10, // Number of results needed
            {
                lambda: 0.025,
                contextConfig: {
                    sentencesAfter: 1,
        ...
        test.skip("addFiles", async () => {
            ...
            const bitcoinBuffer = fs.readFileSync("../examples/src/document_loaders/example_data/bitcoin.pdf");
            vectaraFiles.push({
            ...
            expect(searchResults[0].pageContent).toContain("A Peer-to-Peer Electronic Cash System");
        });
