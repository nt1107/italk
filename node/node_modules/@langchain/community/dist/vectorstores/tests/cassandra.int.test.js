...
describe("CassandraStore - no explicit partition key", () => {
    ...
    test("CassandraStore.fromText", async () => {
        const vectorStore = await CassandraStore.fromTexts(["I am blue", "Green yellow purple", "Hello there hello"], [
            { id: 2, name: "Alex" },
            { id: 1, name: "Scott" },
            { id: 3, name: "Bubba" },
        ], new OpenAIEmbeddings(), noPartitionConfig);
    ...
    test("CassandraStore.fromExistingIndex", async () => {
        await CassandraStore.fromTexts(["Hey", "Whats up", "Hello"], [
            { id: 2, name: "Alex" },
            { id: 1, name: "Scott" },
            { id: 3, name: "Bubba" },
        ], new OpenAIEmbeddings(), noPartitionConfig);
        const vectorStore = await CassandraStore.fromExistingIndex(new OpenAIEmbeddings(), noPartitionConfig);
    ...
    test("CassandraStore.fromExistingIndex (with filter)", async () => {
        ...
        await CassandraStore.fromTexts(["Hey", "Whats up", "Hello"], [
            ...
            { id: 1, name: "Scott" },
            { id: 3, name: "Bubba" },
        ], new OpenAIEmbeddings(), testConfig);
        const vectorStore = await CassandraStore.fromExistingIndex(new OpenAIEmbeddings(), testConfig);
    ...
    test("CassandraStore.addDocuments (with batch))", async () => {
        ...
        const results = await vectorStore.similaritySearch("something called Ulysses", 1);
        expect(results).toEqual([
            new Document({
                pageContent: "So he reads to us from something called Ulysses",
                metadata: { id: 12, name: "Dana" },
    ...
    test("CassandraStore.mmr", async () => {
        const vectorStore = await CassandraStore.fromTexts(["I am blue!", "I am blue!", "I am yellow"], [
            { id: 2, name: "Alex" },
            { id: 1, name: "Scott" },
            { id: 3, name: "Bubba" },
        ], new OpenAIEmbeddings(), noPartitionConfig);
...
describe.skip("CassandraStore - with explicit partition key", () => {
    ...
    test("CassandraStore.partition with cluster filter", async () => {
        const vectorStore = await CassandraStore.fromTexts(["Apple", "Banana", "Cherry", "Date", "Elderberry"], [
        ...
        await expect(vectorStore.similaritySearch("Banana", 1, [
            { name: "group", value: 1 },
...
describe("CassandraStore - with explicit partition key", () => {
    ...
    test("no metadata and no primary keys", async () => {
        const store = await CassandraStore.fromTexts(["I am blue", "Green yellow purple", "Hello there hello"], [], new OpenAIEmbeddings(), {
            ...cassandraConfig,
            dimensions: 1536,
        });
