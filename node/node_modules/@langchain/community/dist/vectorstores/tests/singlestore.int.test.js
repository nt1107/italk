...
class MockEmbeddings extends OpenAIEmbeddings {
    constructor() {
        super();
        Object.defineProperty(this, "queryIndex", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
    ...
    async embedDocuments(documents) {
        return documents.map((text, _) => this.embed(text));
    }
    embed(_) {
        this.queryIndex += 1;
        return [
...
test.skip("SingleStorevectorStore wrong search type", async () => {
    ...
    expect(process.env.SINGLESTORE_PASSWORD).toBeDefined();
    expect(process.env.SINGLESTORE_DATABASE).toBeDefined();
    const vectorStore = await SingleStoreVectorStore.fromTexts([], [], new MockEmbeddings(), {
        connectionURI: `http://${process.env.SINGLESTORE_USERNAME}:${process.env.SINGLESTORE_PASSWORD}@${process.env.SINGLESTORE_HOST}:${process.env.SINGLESTORE_PORT}/${process.env.SINGLESTORE_DATABASE}`,
    ...
    for (const searchType of [
        ...
        await expect(vectorStore.similaritySearch("hello world", 1)).rejects.toThrow("Full text index is required for text-based search strategies.");
    }
    await vectorStore.end();
});
test.skip("SingleStoreVectorStore no filter threshold type 1", async () => {
    ...
    await expect(vectorStore.similaritySearch("hello world", 1, { id: 1 })).rejects.toThrow("Filter threshold is required for filter-based search strategies.");
    await vectorStore.end();
});
...
test.skip("SingleStoreVectorStore no weight coefs 1", async () => {
    ...
    await expect(vectorStore.similaritySearch("hello world", 1, { id: 1 })).rejects.toThrow("Text and vector weight and vector select count multiplier are required for weighted sum search strategy.");
    await vectorStore.end();
});
...
test.skip("SingleStoreVectorStore text only search", async () => {
    ...
    expect(process.env.SINGLESTORE_PASSWORD).toBeDefined();
    expect(process.env.SINGLESTORE_DATABASE).toBeDefined();
    const vectorStore = await SingleStoreVectorStore.fromTexts(weatherTexts, weatherMetadata, new MockEmbeddings(), {
        connectionURI: `http://${process.env.SINGLESTORE_USERNAME}:${process.env.SINGLESTORE_PASSWORD}@${process.env.SINGLESTORE_HOST}:${process.env.SINGLESTORE_PORT}/${process.env.SINGLESTORE_DATABASE}`,
...
test.skip("SingleStoreVectorStore filter by vector search", async () => {
    ...
    const output = await vectorStore.similaritySearch("rainstorm in parched desert, rain", 1, { group: "b" });
    await vectorStore.end();
    expect(output.length).toEqual(1);
...
test.skip("SingleStoreVectorStore weighted sum search unsupported strategy", async () => {
    ...
    await expect(vectorStore.similaritySearch("some text", 1)).rejects.toThrow("Weighted sum search strategy is only available for DOT_PRODUCT distance metric.");
    await vectorStore.end();
});
test.skip("SingleStoreVectorStore weighted sum search", async () => {
    ...
    const output = await vectorStore.similaritySearch("rainstorm in parched desert, rain", 1, { category: "snow" });
    await vectorStore.end();
    expect(output.length).toEqual(1);
    expect(output[0].pageContent).toContain("Atop the rugged peaks, snow fell with an unyielding");
});
