...
test.skip("Test Llama_CPP", async () => {
    ...
    for (let i = 0; i < 5; i += 1) {
        const res = await model.invoke("Where do Pandas live?");
        expect(res).not.toContain("You are a helpful, respectful and honest assistant.");
    }
}, 100000);
test.skip("Test Llama_CPP", async () => {
...
// gbnf grammer test
const gbnfListGrammer = 'root ::= item+ # Excludes various line break characters item ::= "- " [^\r\n\x0b\x0c\x85\u2028\u2029]+ "\n"';
test.skip("Test Llama_CPP", async () => {
    const model = new LlamaCpp({ modelPath: llamaPath, gbnf: gbnfListGrammer });
    const res = await model.invoke("Can you give me a list of 3 cute llama names?");
    console.log(res);
}, 100000);
// JSON schema test
const schemaJSON = {
