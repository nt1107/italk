...
describe.skip("Neo4j Graph Tests", () => {
    ...
    test("Test that neo4j correctly import graph document.", async () => {
        ...
        const output = await graph.query("MATCH (n) RETURN labels(n) AS label, count(*) AS count ORDER BY label");
    ...
    test("Test that neo4j correctly import graph document with base_entity.", async () => {
        ...
        const output = await graph.query("MATCH (n) RETURN apoc.coll.sort(labels(n)) AS label, count(*) AS count ORDER BY label");
        expect(output).toEqual([
...
describe.skip("Neo4j Graph with custom config", () => {
    ...
    test("Test database timeout", async () => {
        ...
        const res = await graphWithTimeout.query("UNWIND range(0,10000,1) AS i MERGE (f:Foo {id:i}) RETURN collect(i)[..5]");
