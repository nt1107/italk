...
test.skip("Vectara Self Query Retriever Test", async () => {
    ...
    const vectorStore = await VectaraStore.fromDocuments(docs, new FakeEmbeddings(), config);
    ...
    const query1 = await selfQueryRetriever.getRelevantDocuments("I want to watch a movie rated higher than 8.5");
    const query2 = await selfQueryRetriever.getRelevantDocuments("Which movies are directed by Greta Gerwig?");
    const query3 = await selfQueryRetriever.getRelevantDocuments("Which movies are either comedy or science fiction and are rated higher than 8.5?");
    const query4 = await selfQueryRetriever.getRelevantDocuments("Wau wau wau wau hello gello hello?");
