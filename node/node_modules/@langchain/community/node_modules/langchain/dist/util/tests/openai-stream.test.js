...
const mockFetchForOpenAIStream = async ({ chunks, status, contentType, }) => {
    ...
    // Mock Fetch API call.
    jest.spyOn(global, "fetch").mockImplementation(async () => new Response(stream, {
        status,
        headers: {
    ...
    const resp = await fetchAdapter({
        ...
        onmessage: (event) => {
            if (event.data?.trim?.() === "[DONE]") {
                ...
                receivedChunks.push(JSON.parse(event.data).choices[0].delta.content ?? "");
            }
        },
    }).catch((err) => {
        error = err;
    ...
    return { resp, receivedChunks, error, done };
};
describe("OpenAI Stream Tests", () => {
    it("should return a 200 response chunk by chunk", async () => {
        ...
        const { resp, receivedChunks, error, done } = await mockFetchForOpenAIStream({
            status: 200,
    ...
    it("should throw the generic error if non-stream content is detected", async () => {
        ...
        expect(resp).toEqual(null);
        expect(error?.message).toBe("Expected content-type to be text/event-stream, Actual: text/plain");
        expect(receivedChunks).toEqual([]);
    });
});
