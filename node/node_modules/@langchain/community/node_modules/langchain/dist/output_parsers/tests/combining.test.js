...
test("CombiningOutputParser", async () => {
    const parser = new CombiningOutputParser(StructuredOutputParser.fromNamesAndDescriptions({
        url: "A link to the resource",
    }), new RegexParser(/Confidence: (A|B|C), Explanation: (.*)/, ["confidence", "explanation"], "noConfidence"));
    ...
    "
  `);
    expect(await parser.parse(`-----Output 0-----
...
-----`)).toMatchInlineSnapshot(`
  ...
  `);
    expect(await parser.parse('```\n{"url": "https://en.wikipedia.org/wiki/Paris"}\n```\nConfidence: A, Explanation: Because it is the capital of France.')).toMatchInlineSnapshot(`
    {
      "confidence": "A",
      "explanation": "Because it is the capital of France.",
      "url": "https://en.wikipedia.org/wiki/Paris",
